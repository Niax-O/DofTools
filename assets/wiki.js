...
function canonicalSidebar(){if(!isPage||!side)return;const nav=side.querySelector('nav');if(!nav)return;const file=location.pathname.split('/').pop();nav.innerHTML=navGroups.map(([title,links])=>`<div class="nav-title">${title}</div>${links.map(([ico,label,href])=>`<a class="${href===file?'active':''}" href="${href}">${ico} ${label}</a>`).join('')}`).join('');const foot=side.querySelector('.sidefoot');if(foot)foot.innerHTML='<b>DOFTOOLS WIKI</b><span>Version 1.3.0</span><span>Guides pratiques</span>';}
canonicalSidebar();
